...
public class FromPlaneToLuggage {

    private final static List<Long> planeToLuggageRoute;
    ...
    static {
        ...
        planeToLuggageRoute = new ArrayList<>(
        ...
        planeToLuggageRoute.addAll(from);
        planeToLuggageRoute.addAll(gates);
    }
    ...
    public Predicate<GraphPoint> getRoute(long initialPoint, long endPoint) {
        return graphPoint -> (graphPoint.getStatus() == Status.EMPTY || graphPoint.getId() == initialPoint || graphPoint.getId() == endPoint)
                             && planeToLuggageRoute.contains(graphPoint.getId());
    }

    public Predicate<GraphPoint> getRouteAnyway(long initialPoint, long endPoint) {
        return graphPoint -> planeToLuggageRoute.contains(graphPoint.getId());
    }
